...
import AtenAst2100DecoderWrapper from "./decoders/aten/atenast2100.js";
import AtenHermonDecoder from "./decoders/aten/hermon.js";
import XK2HID from "./input/keysym2hid.js";
...
export default class RFB extends EventTargetMixin {
    constructor(target, urlOrChannel, options) {
        ...
        // ATEN iKVM decoders
        this._decoders[encodings.encodingAtenAST2100] = new AtenAst2100DecoderWrapper();
        this._decoders[encodings.encodingAtenHermon] = new AtenHermonDecoder();
        // encodingAtenHermon also handles encoding 0x00 from ATEN (which lies about encoding)
        this._decoders[0] = this._decoders[encodings.encodingRaw]; // keep raw as default for 0
    ...
    sendKey(keysym, code, down) {
        ...
        // ATEN iKVM uses HID keycodes
        if (this._rfbAtenIKVM) {
            const hid = XK2HID[keysym];
            if (hid !== undefined) {
                RFB.messages.atenKeyEvent(this._sock, hid, down);
            }
            return;
        }
    ...
    _negotiateSecurity() {
        if (this._rfbVersion >= 3.7) {
            ...
            // Store for ATEN detection heuristic
            this._rfbServerSecurityTypes = Array.from(types);
    ...
    _negotiateAtenAuth() {
        ...
        if (this._rfbTightVNC) {
            // We already read 4 bytes as numTunnels, skip remaining 20
            this._rfbTightVNC = false;
            this._sock.rQskipBytes(20);
        } else {
            // Skip all 24 bytes of ATEN handshake data
            this._sock.rQskipBytes(24);
        }

        // Send credentials: 24-byte null-padded username + 24-byte null-padded password
        const username = this._rfbCredentials.username;
        const password = this._rfbCredentials.password;

        const credBuf = new Uint8Array(48);
        for (let i = 0; i < Math.min(username.length, 24); i++) {
            credBuf[i] = username.charCodeAt(i);
        }
        for (let i = 0; i < Math.min(password.length, 24); i++) {
            credBuf[24 + i] = password.charCodeAt(i);
        }

        this._sock.sQpushBytes(credBuf);
        this._sock.flush();

        Log.Info("ATEN iKVM authentication sent");
        this._rfbInitState = 'SecurityResult';
        return true;
    }
    ...
    _negotiateTightAuth() {
        ...
        if (!this._rfbTightVNC) {  // first pass, do the tunnel negotiation
            ...
            // ATEN detection heuristic #0: older Winbond/Nuvoton or Renesas BMC
            // ATEN servers advertise only security type 0x10 and send garbage
            // tunnel count values (very large or <= 0)
            const onlyTight = this._rfbServerSecurityTypes &&
                this._rfbServerSecurityTypes.length === 1 &&
                this._rfbServerSecurityTypes[0] === securityTypeTight;
            if (this._rfbVersion >= 3.8 && onlyTight &&
                    (numTunnels <= 0 || numTunnels > 0x1000000)) {
                Log.Info('Detected ATEN iKVM server (heuristic #0 - older BMC).');
        ...
        // ATEN detection heuristic #1: newer AST2400 BMC
        // These get past the tunnel phase but send bogus sub-auth counts
        if (this._rfbVersion >= 3.8 && numTunnels === 0 &&
                (subAuthCount === 0 || (subAuthCount & 0xFFFF) === 0x0100)) {
            const onlyTight = this._rfbServerSecurityTypes &&
                this._rfbServerSecurityTypes.length === 1 &&
                this._rfbServerSecurityTypes[0] === securityTypeTight;
            if (onlyTight) {
                Log.Info('Detected ATEN iKVM server (heuristic #1 - newer AST2400 BMC).');
                return this._negotiateAtenAuth();
            }
        }
    ...
    _negotiateServerInit() {
        // ATEN iKVM sends 12 extra bytes in the server init
        const minBytes = this._rfbAtenIKVM ? 36 : 24;
        if (this._sock.rQwait("server initialization", minBytes)) { return false; }
        ...
        // ATEN iKVM has extra bytes after the name
        if (this._rfbAtenIKVM) {
            this._sock.rQskipBytes(8); // unknown
            this._sock.rQshift8(); // IKVMVideoEnable
            this._sock.rQshift8(); // IKVMKMEnable
            this._sock.rQshift8(); // IKVMKickEnable
            this._sock.rQshift8(); // VUSBEnable
        }
        ...
        if (this._rfbAtenIKVM) {
            // ATEN iKVM: start with large size, actual size comes from first FBU
            Log.Info("ATEN iKVM mode: using initial large framebuffer, will resize on first update");
            this._resize(width || 1024, height || 768);
            // Don't send pixelFormat - ATEN ignores it and lies about depth
            this._sendEncodings();
    ...
    _sendEncodings() {
        ...
        // ATEN iKVM uses its own encodings
        if (this._rfbAtenIKVM) {
            encs.push(encodings.encodingAtenAST2100);
            encs.push(encodings.encodingAtenASTJPEG);
            encs.push(encodings.encodingAtenHermon);
            encs.push(encodings.encodingAtenYarkon);
            encs.push(encodings.encodingAtenPilot3);
    ...
    _normalMsg() {
        ...
        // ATEN iKVM proprietary messages
        if (this._rfbAtenIKVM) {
            switch (msgType) {
                case 4:  // Front Ground Event
                    if (this._sock.rQwait("ATEN FGE", 20)) { return false; }
                    this._sock.rQskipBytes(20);
                    return true;
                case 22:  // Keep Alive Event
                    if (this._sock.rQwait("ATEN KA", 1)) { return false; }
                    this._sock.rQskipBytes(1);
                    return true;
                case 51:  // Video Get Info
                    if (this._sock.rQwait("ATEN VGI", 4)) { return false; }
                    this._sock.rQskipBytes(4);
                    return true;
                case 55:  // Mouse Get Info
                    if (this._sock.rQwait("ATEN MGI", 2)) { return false; }
                    this._sock.rQskipBytes(2);
                    return true;
                case 57:  // Session Message
                    if (this._sock.rQwait("ATEN SM", 264)) { return false; }
                    this._sock.rQskipBytes(264);
                    return true;
                case 60:  // Get Viewer Lang
                    if (this._sock.rQwait("ATEN GVL", 8)) { return false; }
    ...
    _framebufferUpdate() {
        ...
        while (this._FBU.rects > 0) {
            if (this._FBU.encoding === null) {
                ...
                // ATEN iKVM uses encoding 0x00 when it means HERMON (0x59)
                if (this._rfbAtenIKVM && this._FBU.encoding === 0x00) {
...
RFB.messages = {
    ...
    // ATEN iKVM key event (18 bytes, HID keycodes)
    atenKeyEvent(sock, hidCode, down) {
        sock.sQpush8(4);   // msg-type
        sock.sQpush8(0);   // padding
        sock.sQpush8(down ? 1 : 0);
        sock.sQpush8(0);   // padding
        sock.sQpush8(0);   // padding
        sock.sQpush8((hidCode >> 24) & 0xFF);
        sock.sQpush8((hidCode >> 16) & 0xFF);
        sock.sQpush8((hidCode >> 8) & 0xFF);
        sock.sQpush8(hidCode & 0xFF);
        // 9 bytes padding
        for (let i = 0; i < 9; i++) {
            sock.sQpush8(0);
        }
    ...
    // ATEN iKVM pointer event (18 bytes)
    atenPointerEvent(sock, x, y, mask) {
        sock.sQpush8(5);   // msg-type
        sock.sQpush8(0);   // padding
        sock.sQpush8(mask & 0xFF);
        sock.sQpush8((x >> 8) & 0xFF);
        sock.sQpush8(x & 0xFF);
        sock.sQpush8((y >> 8) & 0xFF);
        sock.sQpush8(y & 0xFF);
        // 11 bytes padding
        for (let i = 0; i < 11; i++) {
            sock.sQpush8(0);
